...
const END_POINT_MEAL_DATA = 'https://www.themealdb.com/api/json/v1/1/lookup.php?i=';
...
const END_POINT_DRINK_DATA = 'https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=';
...
export const requestMealData = async (id) => {
  const requestFech = await fetch(`${END_POINT_MEAL_DATA}${id}`);
  const requestJson = await requestFech.json();
  return requestJson;
};

export const requestDrinkData = async (id) => {
  const requestFech = await fetch(`${END_POINT_DRINK_DATA}${id}`);
  const requestJson = await requestFech.json();
  return requestJson;
};
